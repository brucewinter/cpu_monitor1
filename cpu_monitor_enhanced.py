...
class CPUMonitorApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Enhanced CPU Monitor & App Restarter")
        self.root.geometry("1000x800")  # Increased size to ensure all elements fit
    ...
    def setup_ui(self):
        ...
        startup_delay_frame.pack(fill="x", padx=20, pady=10)
        ...
        monitoring_startup_delay_frame.pack(fill="x", padx=20, pady=10)
        ...
        columns = ("App Name", "Process Name", "Status", "Enabled", "Last CPU %", "Restart Count")
    ...
    def start_monitoring(self):
        try:
            ...
            messagebox.showerror("Error", "Please enter valid numbers for threshold, interval, and delays")
            return
    ...
    def update_monitoring_info(self):
        if self.monitoring and not self.paused:
            ...
            self.monitoring_info_label.config(
                text=f"Monitoring {enabled_count}/{total_count} applications | "
        ...
        for proc in psutil.process_iter(["pid", "name", "cpu_percent", "exe"]):
            try:
                ...
                if (app_name.lower() in proc_name_lower or 
                    ...
                    # Get CPU usage with proper initialization
                    try:
                        ...
                        # Log for debugging
                        if cpu > 0:
                            self.log_message(f"Process {proc_name} (PID: {proc.info['pid']}) CPU: {cpu:.1f}%")
        ...
        try:
            if os.path.exists("monitored_apps.json"):
                with open("monitored_apps.json", "r") as f:
                    ...
                    # Ensure all apps have the "enabled" field
                    for app in self.monitored_apps:
...
def main():
    ...
    x = (root.winfo_screenwidth() // 2) - 500  # Center 1000x800 window
